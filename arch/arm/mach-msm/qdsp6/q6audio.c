...
// LGE_DOM_UPDATE_S eklee.lee@lge.com 2010/08/02 {
#include  <mach/lg_diag_testmode.h>
// LGE_DOM_UPDATE_E eklee.lee@lge.com 2010/08/02 }
...
static struct q6_hw_info q6_audio_hw[Q6_HW_COUNT] = {
	[Q6_HW_HANDSET] = {
		.min_gain = -1250,//CONFIG_LGE_AUDIO_TUNNING eklee -2000 -> 0 
		.max_gain = 982,
	},
	[Q6_HW_HEADSET] = {
		.min_gain = -1000,
	...
	[Q6_HW_TTY] = {
		.min_gain = 0, //CONFIG_LGE_AUDIO_TUNNING eklee -2000 -> 0 
		.max_gain = 0,
	},
	[Q6_HW_BT_SCO] = {
		.min_gain = -1503,
...
#ifdef CONFIG_LGE_AUDIO_TUNNING
void q6_get_volume(int16_t *vol)
{
	struct q6_hw_info *hw;
	int i, j=0;
	
	for (i= 0 ; i<Q6_HW_COUNT;i++) {
		hw = &q6_audio_hw[i];
    	vol[j++] = (int16_t)hw->min_gain;
	}

	j = Q6_HW_COUNT;
	for (i= 0 ; i<Q6_HW_COUNT;i++) {
		hw = &q6_audio_hw[i];
    	vol[j++] = (int16_t)hw->max_gain;
	}
}  

void q6_set_volume(int16_t *vol)
{
	struct q6_hw_info *hw;
	int i, j=0;
...
#ifdef CONFIG_LGE_AUDIO_CAD_ADD			
// LGE_DOM_UPDATE_S jin333.kim@lge.com 2010/05/07 {
		if(ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_REC_MIC==device_id)
			device_id=ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_MIC;

		if(ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_VR_MIC==device_id)
			device_id=ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_MIC;

		if(ADSP_AUDIO_DEVICE_ID_BT_VR_MIC==device_id)
...
#if 0
static int audio_tune_rx_volume(struct audio_client *ac,int32_t volume)
{
	struct adsp_set_dev_volume_command rpc;
	
	if (ac_control ==0) return -ENOENT;
	
	memset(&rpc, 0, sizeof(rpc));
	rpc.hdr.opcode = ADSP_AUDIO_IOCTL_CMD_SET_DEVICE_VOL;
	rpc.device_id = audio_rx_device_id;
	rpc.path = ADSP_PATH_RX;
	rpc.volume = volume;
	pr_info("audio_rx_volume:%d\n",volume);
	return audio_ioctl(ac_control, &rpc, sizeof(rpc));
}
#endif 
...
#ifdef CONFIG_LGE_AUDIO_CAD_ADD				
// LGE_DOM_UPDATE_S jin333.kim@lge.com 2010/05/07 {
		if(ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_REC_MIC==dev_id)
			dev_id=ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_MIC;

		if(ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_VR_MIC==dev_id)
			dev_id=ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_MIC;

		if(ADSP_AUDIO_DEVICE_ID_BT_VR_MIC==dev_id)
			dev_id=ADSP_AUDIO_DEVICE_ID_BT_SCO_MIC;

// LGE_DOM_UPDATE_E jin333.kim@lge.com 2010/05/07 }
#endif
		rpc.new_device = dev_id;
	}
	rpc.device_class = 0;
...
static void audio_rx_analog_enable(int en)
{
	switch (audio_rx_device_id) {
		...
		case ADSP_AUDIO_DEVICE_ID_TTY_HEADSET_SPKR:
			if (analog_ops->tty_headset_enable)
				analog_ops->tty_headset_enable(en);
			break;
		...
		case ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_STEREO_W_STEREO_HEADSET:
			if (analog_ops->sp_hp_enable)
				analog_ops->sp_hp_enable(en);
			break;
		...
		case ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_STEREO:
			if (analog_ops->stereo_speaker_enable)
				analog_ops->stereo_speaker_enable(en);
			break;
...
#ifdef CONFIG_LGE_DOCK_STATE_DETECT
extern void analog_speaker_select_path(void);

void audio_rx_analog_dock_update(int en)
{

	pr_info("[1]audio_rx_analog_dock_update--en:%d, audio_rx_device_id:%08x\n",en,audio_rx_device_id);

	switch (audio_rx_device_id) {
		case ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_MONO:
			
			pr_info("[2]audio_rx_analog_dock_update %p\n",analog_ops->speaker_enable);
			
			if (analog_ops->speaker_enable) {
				pr_info("[3]audio_rx_analog_dock_update %p\n",analog_ops->speaker_enable);
...
#ifdef CONFIG_LGE_AUDIO_CAD_ADD
    case ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_REC_MIC:	
	case ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_VR_MIC:		
#endif 
...
#ifdef CONFIG_LGE_AUDIO_CAD_ADD
	case ADSP_AUDIO_DEVICE_ID_BT_VR_MIC:
#endif		
		if (analog_ops->bt_sco_enable)
			analog_ops->bt_sco_enable(en);
		break;
...
// LGE_DOM_UPDATE_S eklee.lee@lge.com 2010/08/02 {
	if(testmode_volume_state == VOL_LEV_OFF){
		level=0;
	}else if(testmode_volume_state == VOL_LEV_MIN){
		level=48;
	}else if(testmode_volume_state == VOL_LEV_MEDIUM){
		level=70;
	}else if(testmode_volume_state == VOL_LEV_MAX){
		level=90;
	}
// LGE_DOM_UPDATE_E eklee.lee@lge.com 2010/08/02  }	


	pr_info("[%s:%s] q6audio_set_rx_volume ADIE level:%d\n", __MM_FILE__, __func__,level);
...
//LGE_COMMENT start
struct audio_client *q6audio_boot_open_pcm(uint32_t bufsz, uint32_t rate,

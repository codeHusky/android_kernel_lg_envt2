...
#ifdef LGE_FW_LU2300
// col output <-> row output
static unsigned int keypad_row_gpios[] = { 34 };
static unsigned int keypad_col_gpios[] = { 40 };

// LGE_DOM_UPDATE_S itsuki@lge.com 2009/12/03 {
static unsigned int lu2300_rev0_keypad_row_gpios[] = { 34 };
static unsigned int lu2300_rev0_keypad_col_gpios[] = { 41, 40 };
// LGE_DOM_UPDATE_E itsuki@lge.com 2009/12/03 }

// LGE_DOM_UPDATE_S namkyu2.kang@lge.com 2010/02/27 {
static unsigned int lu2300_revD_keypad_row_gpios[] = { MSM_KEY_ROW_IN_0 };
static unsigned int lu2300_revD_keypad_col_gpios[] = { MSM_KEY_COLUMN_OUT_0,MSM_KEY_COLUMN_OUT_1  };
// LGE_DOM_UPDATE_E namkyu2.kang@lge.com 2010/02/27 }

#elif defined LGE_FW_LU940
// col <-> row
static unsigned int keypad_row_gpios[] = { 34, 33, 32 }; // row_out
static unsigned int keypad_col_gpios[] = { 42, 41, 40 };   // col_in

#elif defined (LGE_FW_SU950) || defined (LGE_FW_KU950) || defined (LGE_FW_KU9500)
// col <-> row
#define KEY_DMB						KEY_F13
#define KEY_CAMERAFOCUS 252
static unsigned int keypad_row_gpios[] = { 41, 40, 39 }; // row_in
static unsigned int keypad_col_gpios[] = { 36, 35, 34 };  // col_out

#elif defined CONFIG_LGE_MACH_ENVT2
// col <-> row
static unsigned int keypad_row_gpios[] = { 41, 40 }; // row_in
static unsigned int keypad_col_gpios[] = { 36, 35 };   // col_out
...
// LGE_DOM_UPDATE_S jiyoung.song@lge.com 2010/02/20 {
#ifdef CONFIG_LGE_MACH_ENVT2
/* in case of col_out and row_in */
#define STERN_KEYMAP_INDEX(col, row) ((col)*ARRAY_SIZE(keypad_row_gpios) + (row))
#endif
// LGE_DOM_UPDATE_E jiyoung.song@lge.com 2010/02/20 }
...
static const unsigned short keypad_keymap_surf[ARRAY_SIZE(keypad_col_gpios) *
	...
	[KEYMAP_INDEX(0, 0)] = KEY_ENTER,       /*  OK */
	[KEYMAP_INDEX(0, 1)] = KEY_MENU,		/* Camera */
	
	[KEYMAP_INDEX(1, 0)] = KEY_VOLUMEUP,	/* Volume up */
	[KEYMAP_INDEX(1, 1)] = KEY_VOLUMEDOWN,	/* Volume down */

	[KEYMAP_INDEX(2, 0)] = KEY_BACK,		/* Send */
	[KEYMAP_INDEX(2, 1)] = KEY_BACK,// KEY_HOME,		/* Clear */

#elif defined (LGE_FW_SU950) || defined (LGE_FW_KU950) || defined (LGE_FW_KU9500)
/* This following code is only valid */

	[STERN_KEYMAP_INDEX(0, 0)] = KEY_HOME,					/* HOME = 102 */
	[STERN_KEYMAP_INDEX(0, 1)] = KEY_BACK,					/* BACK = 158  */
	[STERN_KEYMAP_INDEX(0, 2)] = KEY_SEARCH,				/* SEARCH = 217 */
	
	[STERN_KEYMAP_INDEX(1, 0)] = KEY_MENU,					/* MENU = 139 */
	[STERN_KEYMAP_INDEX(1, 1)] = KEY_VOLUMEUP,  		/* Volume up =  115 */
	[STERN_KEYMAP_INDEX(1, 2)] = KEY_VOLUMEDOWN,	/* Volume down = 114 */

	[STERN_KEYMAP_INDEX(2, 0)] = KEY_DMB,					/* DMB = KEY_F13 = 183 */
	[STERN_KEYMAP_INDEX(2, 1)] = KEY_CAMERA,				/* CAM1 */
	[STERN_KEYMAP_INDEX(2, 2)] = KEY_CAMERAFOCUS,	/* CAM2 */
#elif defined CONFIG_LGE_MACH_ENVT2
...
static const unsigned short keypad_keymap_ffa[ARRAY_SIZE(keypad_col_gpios) *
	...
	[KEYMAP_INDEX(0, 0)] = KEY_SEND,       /*  SEND */
	[KEYMAP_INDEX(0, 1)] = KEY_MENU,		/* MENU */
	[KEYMAP_INDEX(0, 2)] = KEY_BACK,		/* BACK */
	
	[KEYMAP_INDEX(1, 0)] = KEY_HOME,	/* HOME */
	[KEYMAP_INDEX(1, 1)] = KEY_VOLUMEUP,	/* Volume up */
	[KEYMAP_INDEX(1, 2)] = KEY_VOLUMEDOWN,	/* Volume down */

	[KEYMAP_INDEX(2, 0)] = KEY_ENTER,		/* TBD */
	[KEYMAP_INDEX(2, 1)] = KEY_BACK,		/* CAM1 */
	[KEYMAP_INDEX(2, 1)] = KEY_BACK,		/* CAM2*/

#elif defined CONFIG_LGE_MACH_ENVT2
	[KEYMAP_INDEX(0,0)] = KEY_HOME,
	[KEYMAP_INDEX(0,1)] = KEY_SEND,
	[KEYMAP_INDEX(1,0)] = KEY_MENU,
	[KEYMAP_INDEX(1,1)] = KEY_BACK,
#else // LGE_FW_LU2300
...
#if defined (LGE_FW_SU950) || defined (LGE_FW_KU950) || defined (LGE_FW_KU9500) || defined (CONFIG_LGE_MACH_ENVT2)

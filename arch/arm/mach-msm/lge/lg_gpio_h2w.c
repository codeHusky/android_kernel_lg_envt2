...
static DEFINE_SPINLOCK(gpio_hw_lock);
...
#ifdef MIC_TEST 
	struct hrtimer mic_bias_timer;
	ktime_t mic_bias_time;
#endif	
};
...
static void insert_headset(void)
{
	...
	H2W_DBG("");
#ifdef CONFIG_LGE_AUDIO_HEADSET_PROTECT
	spin_lock(&gpio_hw_lock);
	gpio_set_value(hi->gpio_mic_bias_en, 1);
	spin_unlock(&gpio_hw_lock);
#ifndef MIC_TEST	
	msleep(120);
#endif
#endif 	
#ifdef MIC_TEST 
	hrtimer_start(&hi->mic_bias_timer, hi->mic_bias_time, HRTIMER_MODE_REL);
#endif 
#ifndef MIC_TEST	
...
static enum hrtimer_restart mic_bias_event_timer_func(struct hrtimer *data)
{
	unsigned long irq_flags;
...
#ifdef MIC_TEST 
	hi->mic_bias_time = ktime_set(0, 120000000); /* 100 ms */
#endif 	   
...
#ifdef MIC_TEST 
	hrtimer_init(&hi->mic_bias_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	hi->mic_bias_timer.function = mic_bias_event_timer_func;
#endif

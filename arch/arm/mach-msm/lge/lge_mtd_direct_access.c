...
#ifdef CONFIG_LGE_APPS_FACTORY_RESET
static int test_read_val = 0;
extern char isFactoryFlagChanged;
#endif
...
//FACTORY_RESET
//static int init_mtd_access(int partition, int block);
//int lge_init_mtd_access(int partition, int block);
int lge_init_mtd_access(char *partition_name, int block);
...
static int test_init(void)
{
	...
	return lge_init_mtd_access(PERSIST_PART_NAME, block);
}
...
// if the flag has already been changed before bootcomplete is called, mark it so that post works like disabling adb can be performed.
#ifdef CONFIG_LGE_APPS_FACTORY_RESET
	if((flag==5) &&(test_read_val == 3))
	{
		isFactoryFlagChanged = 1;
		printk(KERN_INFO"%s: flag will be changed from %d to %d\n", __func__, test_read_val, flag);
	}
#endif
...
int lge_read_block(int ebnum, unsigned char *buf)
{
	...
	//memset(buf, 0, mtd->erasesize); // stack overflow
	memset(buf, 0, pgsize);
...
int lge_init_mtd_access(char *partition_name, int block)
{
	...
	err = -ENOMEM;
	bufsize = pgsize * 2;
// somtimes allocation failure occurs

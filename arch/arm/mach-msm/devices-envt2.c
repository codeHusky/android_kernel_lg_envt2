...
#ifdef CONFIG_LGE_MACH_ENVT2_CHECK_HW_REV
// this function makes gpios validated with pcb version
// and return PCB Revision
#include "smd_private.h"
#include <linux/proc_fs.h>

int __init lge_read_pcb_version(void) 
{
  static int *p_smem_lge_pcb_rev ;
  static int lge_pcb_rev ;

  if( !p_smem_lge_pcb_rev ) {
    int size ;
//		static int smem_lge_pcb_rev =3 ;
    p_smem_lge_pcb_rev = smem_get_entry( SMEM_HW_PCB_REV ,&size ) ;

    if( p_smem_lge_pcb_rev == NULL /*  || size != sizeof(int ) == 8AI¡§I¨Ï¨£U..*/ ) { 
      printk("[[[[[[[[[Error on SMEM]]]]]]]") ;
      return -1 ;
    }
    lge_pcb_rev = *p_smem_lge_pcb_rev ;
...
/*
	meminfo = (
	  nr_banks = 0x2,
	  bank = (
		(start = 0x20000000, size = 0x0E800000, node = 0x0, highmem = 0x0),
		(start = 0x30000000, size = 0x0FF00000, node = 0x0, highmem = 0x0),
*/
	struct membank *bank;

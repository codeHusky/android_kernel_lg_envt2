...
typedef enum
{
  ...
  EXTERNAL_MEMORY_USED_SIZE_CHECK_TEST = 4,
...
typedef enum{
  ...
  FACTORY_RESET_IDLE = 6,
  FACTORY_RESET_PRIVACY_MENU = 7,
  FACTORY_RESET_NA = 8,
...
#endif /*ONFIG_LGE_APPS_FACTORY_RESET*/

typedef enum
{
  FIRST_BOOTING_COMPLETED_CHECK,
  FIRST_BOOTING_CHG_MODE_CHECK=0xF, // charging mode check, temporal
}test_mode_req_first_booting_complete_check;

typedef enum
{
  FIRST_BOOTING_COMPLETED,
  FIRST_BOOTING_NOT_COMPLETED
}test_mode_first_booting_complete_type;
...
#if 1//def LG_FW_TEST_MODE_V7_1
  test_mode_req_test_script_mode_type test_mode_test_scr_mode;
#endif /*LG_FW_TEST_MODE_V7_1*/
  test_mode_first_booting_complete_type complete_status_check;
...
typedef union
{
  ...
  char  memory_check;
  unsigned long long socket_memory_size;
#ifdef CONFIG_LGE_CAMERA_TESTMODE
  test_mode_req_cam_type		 camera;
#endif
...
typedef enum
{
  ...
  TEST_MODE_MAC_READ_WRITE = 88, // wifi MAC RW byoungwook.baek@lge.com

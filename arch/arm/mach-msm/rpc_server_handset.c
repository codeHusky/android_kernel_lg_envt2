...
/* LGE_CHANGES_S [woonghee@lge.com] 2010-03-03, [VS740] for multikit detect */
#if defined(CONFIG_LGE_EXT_PWR_CRADDLE)
#define HS_FLIP_OPEN_K 0x9C
#endif
/* LGE_CHANGES_E */
...
#if defined(CONFIG_LGE_ATS_ETA_MTC_KEY_LOGGING)
		ats_eta_key_logging((int)key, (unsigned char)(key_code != HS_REL_K), (int)UMH_WAIT_PROC);
#endif
		break;
...
/* LGE_CHANGES_S [woonghee@lge.com] 2010-03-03, [VS740] for multikit detect */
	...
	case HS_FLIP_OPEN_K: /* MultiKit Detect */
		if(key_parm == HS_FLIP_OPEN_K) { // inserted
			...
			printk(KERN_ERR,"LG_FW : craddle is attached!!!");
		} else {// removed
			hall_ic_dock_report_event(0);  // DOCK_STATE_UNDOCKED
			printk(KERN_ERR,"LG_FW : craddle is detached!!!");
		}
		return ;
#endif
/* LGE_CHANGES_E */
...
// 2010.12.29 jihoon.lee - hs input device for using input_report_key in testmode
#ifdef CONFIG_LGE_DIAG_TESTMODE
static struct input_dev *l_hs_dev;
struct input_dev *hs_get_input_dev(void)
{
	return l_hs_dev;
}
EXPORT_SYMBOL(hs_get_input_dev);
#endif

...
static void mmc_blk_put(struct mmc_blk_data *md)
{
	...
	if (md->usage == 0) {
		...
		blk_cleanup_queue(md->queue.queue); // CONFIG_LGE_MMC_CLEANUP_PATCH - already migrated
...
static int mmc_blk_probe(struct mmc_card *card)
{
 ...
 out:
	mmc_cleanup_queue(&md->queue); // CONFIG_LGE_MMC_CLEANUP_PATCH - already migrated
	mmc_blk_put(md);

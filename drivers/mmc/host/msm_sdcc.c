...
static int msmsdcc_get_status(struct mmc_host *mmc)
{
	struct msmsdcc_host *host = mmc_priv(mmc);
	unsigned int status;
	unsigned long flags; // conflict with msmsdcc_probe(), this function uses irqsave

	//spin_lock_irq(&host->lock);
	spin_lock_irqsave(&host->lock, flags);  // conflict with msmsdcc_probe(), this function uses irqsave
	...
	if (host->plat->status) {
		...
		if (status ^ host->oldstat) {
			...
			// [COMMON] Checking Gpio pin status before initialization of bus
			host->oldstat = status;
			//spin_unlock_irq(&host->lock);
			spin_unlock_irqrestore(&host->lock, flags);  // conflict with msmsdcc_probe(), this function uses irqsave
...
static int msmsdcc_get_ro(struct mmc_host *mmc)
{
	...
	printk("%s: Card read-only status %d\n", __func__, wpswitch_status);
...
// 20110322 wo0gi[byoungwook.baek@lge.com], Remove delay whe Wi-Fi turned off [START]
#if 1
static void
msmsdcc_check_status(unsigned long data)
{
	...
	if (msmsdcc_get_status(host->mmc)) {
		...
		if (host->plat->status_irq == MSM_GPIO_TO_INT(CONFIG_BCM4329_GPIO_WL_RESET)) {
			...
			if( !host->eject )  { 		// When Wi-Fi turned on, eject value=> 0--> delay 150ms
				mmc_detect_change(host->mmc, (3 * HZ) / 20);
				printk(KERN_INFO "Wi-Fi on & delay 150ms\n");
			} else {					// When Wi-Fi turned off, eject value=> 1 --> delay 0ms
				mmc_detect_change(host->mmc, 0);
				printk(KERN_INFO "Wi-Fi on & delay 0ms\n");
			}
...
// status will be shown in msmsdcc_get_status() function
			...
			if ((host->plat->status_irq == MSM_GPIO_TO_INT(CONFIG_BCM4329_GPIO_WL_RESET)) && status) {
				mmc_detect_change(host->mmc, (3 * HZ) / 20);
			}
			else
...
#endif
// 20110322 wo0gi[byoungwook.baek@lge.com], Remove delay whe Wi-Fi turned off [END]
...
#if 1//!defined (CONFIG_LGE_MMC_DETECTION)  //THUNDER_MIGRATION
/* LGE_CHANGES_S [junyeong.han@lge.com] 2010-02-21, add mmc host before request_irq */
...
#else
		if (host->plat->sdiowakeup_irq && host->plat->status_irq != gpio_to_irq(CONFIG_BCM4329_GPIO_WL_RESET))
#endif			

...
/* LGE_CHANGES_S [fred.cho@lge.com ] 2010-01-12, expand timeout for HW_DETECTION */
...
static int __init mmc_init(void)
{
	...
	if (!workqueue)
	{
		printk(KERN_INFO "%s, create workqueue failed\n", __func__);
		return -ENOMEM;
	}
	...
	if (ret)
	{
		printk(KERN_INFO "%s, mmc_register_bus() failed %d, destroy_workqueue\n", __func__, ret);
	...
	if (ret)
	{
		printk(KERN_INFO "%s, mmc_register_host_class() failed %d, unregister_bus\n", __func__, ret);
		goto unregister_bus;
	}
	...
	if (ret)
	{
		printk(KERN_INFO "%s, sdio_register_bus() failed %d, unregister_host_class\n", __func__, ret);

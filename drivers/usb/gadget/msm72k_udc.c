...
extern const u16 lg_android_pid;
extern const u16 lg_factory_pid;
extern u16 product_id;
extern u16 check_cable_id;
#define LG_FACTORY_CABLE_TYPE 3
#define LG_FACTORY_CABLE_130K_TYPE 10
#endif
#endif /*CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB*/

#define TOUCH_COMMON_NOISE
#ifdef TOUCH_COMMON_NOISE
extern void AMRI5K_s_ts_init_delta(u8 plug);
extern void AMRI5K_m_ts_init_delta(u8 plug);
#endif
...
{
/* 2010.8.9 jaeho.cho@lge.com this makes scorpion sleep with LT connection */
#if 0//defined(CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB)

	u16 tempCableID = msm_chg_LG_cable_type();

	if (LG_FACTORY_CABLE_TYPE == tempCableID
			|| LG_FACTORY_CABLE_130K_TYPE == tempCableID)  //detect LT cable
...
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-09-09, apply 0mA charging current */
...
//LGE_5035_PATCH - ulpi_read() has been removed since 5035 CS release but maintain this for other usage
#if defined (CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB)
...
static void usb_reset(struct usb_info *ui)
{
    ...
    /* to supports FS USB in the Factory ( LT cable will be connected) */
#if defined (CONFIG_LGE_USB_SUPPORT_SMEM_CABLE_TYPE)
    cable_type = lgfw_smem_cable_type();
#else
...
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-09-24, workaround to fix unexpected charging without ext_pwr */
				...
				msm72k_pm_qos_update(0);
#ifdef TOUCH_COMMON_NOISE
                AMRI5K_s_ts_init_delta(0);
                AMRI5K_m_ts_init_delta(0);  				
#endif
				wake_unlock(&ui->wlock);
				break;
			}
		...
		case USB_STATE_OFFLINE:
			...
			if ((flags & USB_FLAG_VBUS_ONLINE) && _vbus) {
				...
				usb_do_work_check_vbus(ui);
#ifdef TOUCH_COMMON_NOISE				
                AMRI5K_s_ts_init_delta(1);
                AMRI5K_m_ts_init_delta(1);  
#endif
				ret = request_irq(otg->irq, usb_interrupt,
							IRQF_SHARED,
							ui->pdev->name, ui);
...
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
#ifdef CONFIG_LGE_CHARGING
	lg_udc_debug("LG_FW :: is_usb_online(ui) = [[%d]], online = [[%d]] \n", is_usb_online(ui), online);

    if (!((is_usb_online(ui) ==  1 && online == 1) || (is_usb_online(ui) ==  0 && online == 0)) &&
		is_usb_online(ui) ==  online)
		goto out;
#else
	...
	lg_udc_debug("LG_FW :: vbus_state online = [[%d]] \n", online);
#endif
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
...
/* QCT patch for USB kernel panic */
#ifdef CONFIG_LGE_USB_GADGET_DRIVER
	if (dev->irq) { 
		printk("freeing IRQ\n"); 

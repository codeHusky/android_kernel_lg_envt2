...
#ifdef DEBUG
#define lg_otg_debug(fmt, arg...)	printk(KERN_ERR "%s(): " fmt, __func__, ##arg)
#else
#define lg_otg_debug(fmt, arg...)
#endif
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
...
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
#ifdef CONFIG_LGE_CHARGING
		lg_otg_debug("LG_FW :: - on \n");
        atomic_set(&otg_vbus_flag, 1);
#endif
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
	}
	else {
...
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
#ifdef CONFIG_LGE_CHARGING
		lg_otg_debug("LG_FW :: - off \n");
		atomic_set(&otg_vbus_flag, 0);
#endif
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
...
#ifdef CONFIG_LGE_CHARGING
void msm_otg_start_peripheral_ext(int on)
{
  	struct msm_otg *dev = container_of(&the_msm_otg->otg, struct msm_otg, otg);

	if (!the_msm_otg->otg.gadget)
		return;

	if (on) {
		usb_gadget_vbus_connect(the_msm_otg->otg.gadget);
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */		
#ifdef CONFIG_LGE_CHARGING
		lg_otg_debug("LG_FW :: - on \n");
        atomic_set(&otg_vbus_flag, 1);
...
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
		...
		atomic_set(&dev->chg_type, USB_CHG_TYPE__INVALID);
		usb_gadget_vbus_disconnect(the_msm_otg->otg.gadget);
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
#ifdef CONFIG_LGE_CHARGING
		lg_otg_debug("LG_FW :: - off \n");
		atomic_set(&otg_vbus_flag, 0);
...
#if 0 // ndef BIGLAKE_USB_SLEEP LGE_CHANGES
...
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */
#ifdef CONFIG_LGE_CHARGING
		    lg_otg_debug("LG_FW :: is_b_sess_vld() = [[%d]] \n",is_b_sess_vld());
#endif
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2010-10-19, workaround to fix unexpected charging without ext_pwr */

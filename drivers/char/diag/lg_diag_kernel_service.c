...
#include <linux/timer.h>
#ifdef CONFIG_LGE_DIAG_UTS
#include <mach/lg_diag_udm.h>	//UTS_check_mickey   20100708  [jungwook.youn@lge.com]
#endif
...
PACK (void *)LGF_AudioTest (PACK (void	*)req_pkt_ptr, uint16		pkt_len );
#endif 
#ifdef CONFIG_LGE_DIAG_WMC
PACK (void *)LGF_WMC (PACK (void	*)req_pkt_ptr, uint16 pkt_len);
#endif
#ifdef CONFIG_LGE_ATS_ETA_MTC
PACK (void *)LGF_MTCProcess (PACK (void *)req_pkt_ptr, unsigned short pkt_len);
#endif
#ifdef CONFIG_LGE_DIAG_UTS
PACK (void *)LGF_Udm (PACK (void	*)req_pkt_ptr, uint16		pkt_len ); //UTS_check_mickey   20100708  [jungwook.youn@lge.com]
#endif
#ifdef CONFIG_LGE_DIAG_UTS_KEYPRESS
PACK (void *)LGF_KeyPress (PACK (void	*)req_pkt_ptr, uint16		pkt_len );
#endif
#ifdef CONFIG_LGE_DIAG_UTS_SCREEN_SHOT
...
#endif
#ifdef CONFIG_LGE_AUDIO_TUNE_TOOL
	{DIAG_AUDIO_TEST_F, DIAG_AUDIO_TEST_F, LGF_AudioTest},
#endif
...
#endif
#ifdef CONFIG_LGE_DIAG_UTS
	{DIAG_UDM_SMS_MODE , DIAG_UDM_SMS_MODE , LGF_Udm },		//UTS_check_mickey   20100708  [jungwook.youn@lge.com]
#endif
#ifdef CONFIG_LGE_DIAG_UTS_KEYPRESS
...
#ifdef CONFIG_LGE_DIAG_UTS
udm_sms_status_new lg_diag_req_udm_sms_status_new = {0};	//UTS_check_mickey    [jungwook.youn@lge.com]
#endif
		...
		char *buf)
{
  ...
  printk(KERN_INFO "%s, attr_name : %s, length : %d\n", __func__, attr->attr.name, lg_diag_req_pkt_length);
...
//UTS_check_mickey   20100708  [jungwook.youn@lge.com]
#ifdef CONFIG_LGE_DIAG_UTS
...
//UTS_check_mickey   20100708  [jungwook.youn@lge.com]
	...
	if (ret) {
		printk( KERN_DEBUG "LG_FW : diag device file6 create fail\n");
	...
	if (ret) {
		printk( KERN_DEBUG "LG_FW : diag device file7 create fail\n");
	...
	if (ret) {
		printk( KERN_DEBUG "LG_FW : diag device file8 create fail\n");
	...
	if (ret) {
		printk( KERN_DEBUG "LG_FW : diag device file9 create fail\n");
	...
	if (ret) {
		printk( KERN_DEBUG "LG_FW : diag device file10 create fail\n");
	...
	if (ret) {
		printk( KERN_DEBUG "LG_FW : diag device file11 create fail\n");
...
//UTS_check_mickey    [jungwook.youn@lge.com]
#ifdef CONFIG_LGE_DIAG_UTS
	device_remove_file(&pdev->dev, &dev_attr_sms_status);
	device_remove_file(&pdev->dev, &dev_attr_get_sms);
	device_remove_file(&pdev->dev, &dev_attr_set_sms);
...
/* LGE_CHANGES_S [kyuhyung.lee@lge.com] - #ifdef CONFIG_LGE_DIAG_UTS_SCREEN_SHOT*/

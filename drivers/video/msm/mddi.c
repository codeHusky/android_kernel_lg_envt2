...
static int mddi_sleep(struct platform_device *pdev, pm_message_t state);
static int mddi_wake(struct platform_device *pdev);
#else /*origin*/
#ifdef CONFIG_PM
...
//	pm_qos_update_requirement(PM_QOS_SYSTEM_BUS_FREQ , "mddi",128000);
	pm_qos_update_requirement(PM_QOS_SYSTEM_BUS_FREQ , "mddi",58000);
#endif
...
#else
	if (mfd->index == 0)
		mfd->fb_imgType = MDP_RGB_565;//MSMFB_DEFAULT_TYPE;
	else
...
/* LG_FW : 2010.08.05 jinho.jang - MDDI TYPE1 and TYPE2 Switch */
	...
	ret = mddi_change_to_type_1(MDDI_HOST_PRIM);
	if(ret < 0)

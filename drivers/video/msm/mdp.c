...
/* LG_FW : 2010.03.29 jiho.jang */
//switch dma2 or dma_s
#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA)
//#define MDP_USE_DMA_S
#endif
...
/* LG_FW : 2010.03.27 jiho.jang - Ext_MDDI SBA */
#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA)
	} 
	else if (term == MDP_DMA_E_TERM) 
	{
		mdp_pipe_ctrl(MDP_DMA_E_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
		outpdw(MDP_BASE + 0x004C, 0x0);	/* start DMA */
 	}
#else /*origin*/
	} else if (term == MDP_DMA_E_TERM) {
		mdp_pipe_ctrl(MDP_DMA_E_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
...
//LG_FW : 2010.08.10 jinho.jang
//check whether folder is open or nor, and then confirm return condition
#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA)	
...
#define MAIN_FSYNC	141
#else
#define MAIN_FSYNC	100
#endif

#else
#define MAIN_FSYNC	98
...
static int mdp_probe(struct platform_device *pdev)
{
	...
	case EBI2_PANEL:
		...
		#ifdef MDP_USE_DMA_S // ! CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA
...
/* LG_FW : 2010.03.29 jiho.jang */
#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA)
		if(mfd->panel_info.pdest == DISPLAY_1) //sub fsync		
			mfd->vsync_gpio = SUB_FSYNC;
		
		if(mfd->panel_info.pdest == DISPLAY_2) //main fsync
			mfd->vsync_gpio = MAIN_FSYNC;		
#else /*origin*/
		if (mdp_pdata)

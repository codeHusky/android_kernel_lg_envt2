...
/* LG_FW : 2010.06.30 jiho.jang - Ext MDDI Vsync */
#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA)
#define MDP_SYNC_CFG_2 		0x308 
#define MDP_SYNC_STATUS_2	0x314 
#define MDP_EXT_VSYNC_OUT_CTRL	0x320
#define MDP_EXT_VSYNC_INIT_VAL	0x330
#define MDP_VSYNC_SEL		0x324
#endif
...
/* LG_FW : 2010.06.30 jiho.jang - Ext MDDI Vsync */
		...
		if (mfd->panel_power_on) {
			...
			if(mfd->panel_info.type == EXT_MDDI_PANEL)
				MDP_OUTP(MDP_BASE + MDP_SYNC_STATUS_2, vsync_load_cnt);
			}
#else /*origin*/
		if (mfd->panel_power_on) {
			MDP_OUTP(MDP_BASE + MDP_SYNC_STATUS_0, vsync_load_cnt);
...
/* LG_FW : 2010.06.30 jiho.jang - Ext MDDI Vsync */
#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA)
static void mdp_set_sync_cfg_2(struct msm_fb_data_type *mfd, int vsync_cnt)
{
	unsigned long cfg;
	...
	MDP_OUTP(MDP_BASE + MDP_SYNC_CFG_2, cfg);
}
#endif
...
/* LG_FW : 2010.03.29 jiho.jang - Dual LCD Switch */
	...
	/* LG_FW : 2010.06.30 jiho.jang - Ext MDDI Vsync */
	#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA)
				if(mfd->panel_info.type == MDDI_PANEL) // primay mddi
				{
				...
				if(mfd->panel_info.type == EXT_MDDI_PANEL) //externel mddi
				{
					mdp_set_sync_cfg_2(mfd, vsync_cnt_cfg);
					...
					/* line counter init value at the next pulse */
					MDP_OUTP(MDP_BASE + MDP_EXT_VSYNC_INIT_VAL,
								vsync_load_cnt);
				 	...
				 	* polarity flip
				 	*/
					MDP_OUTP(MDP_BASE + MDP_EXT_VSYNC_OUT_CTRL,
								BIT(30) | BIT(0));

					/* LG_FW : 2010.06.30 jiho.jang - added for Ext MDDI Vsync */
					MDP_OUTP(MDP_BASE + MDP_VSYNC_SEL, 0x14); // Added line to hook DMA_E on GPOI 100

					/* threshold */
					MDP_OUTP(MDP_BASE + 0x208, /* MDP_SYNC_THRESH_2 */
					 	(vsync_above_th << 16) |
					 	(vsync_start_th));

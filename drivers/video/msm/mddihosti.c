...
#endif /* CONFIG_LGE_FB_MSM_MDDI_2 */
#endif

	pad_reg_val = 0x00220020;
#ifdef CONFIG_LGE_FB_MSM_MDDI_2
	pad_reg_val |= 0x10000000;
	pad_io_ctl = 0x00320000;
#endif

/* LG_FW : 2010.08.05 jinho.jang - MDDI PAD CTL REG Setting */
#ifndef FEATURE_MDDI_PAD_CTRL
	//increase power setting - 1
	//pad_reg_val = 0x40880020; // 0
	//pad_reg_val = 0xF1FE0020; // 1	
	pad_io_ctl = 0x00321009;
	pad_reg_val = 0x40880020; //default	
	//pad_reg_val = 0x87119420; //sterngth 50%
#endif

#if defined(CONFIG_FB_MSM_MDP31) || defined(CONFIG_FB_MSM_MDP40)
	mddi_host_reg_out(PAD_IO_CTL, pad_io_ctl/*0x00320000*/);
...
/* LG_FW : 2010.08.05 jinho.jang - MDDI PAD CTL REG Setting */
#ifdef FEATURE_MDDI_PAD_CTRL	
	mddi_host_reg_out(DRIVER_START_CNT, 0xC800C8);
#else
...
void omddi_SendFwLinkSkewCal(mddi_host_type host_idx) 
{ 
	mddi_host_reg_out(CMD, MDDI_CMD_FW_LINK_SKEW_CAL); 
	printk("%s: Skew Calibration done!!\n", __func__); // debug 
}
...
/* Write out the MDDI configuration registers */
{
	...
	if (!initialized) {
		...
		/* Send Skew Calibration Packet */ 
		//omddi_SendFwLinkSkewCal(host_idx);
...
DEFINE_MUTEX(Display_freeze_workaround);

void mddi_SendGetClientStatus(void)
{
	u32 reg, temp, cnt;
	mddi_host_type host_idx = mddi_curr_host;
	
	mutex_lock(&Display_freeze_workaround);
 	mddi_host_reg_out(CMD, MDDI_CMD_POWERDOWN);
	...
	for(cnt = 0; cnt<100; cnt++) {
		if(!(reg & 0x00000010)) { //check link is down
			reg = mddi_host_reg_in(STAT);
		}else {
			break;
		}
	}
	
	temp = mddi_host_reg_in(PAD_CTL);
	temp &= 0x000FFFFF; // type 1
	mddi_host_reg_out(PAD_CTL, temp);
	mddi_host_reg_out(CMD, MDDI_CMD_HIBERNATE);
	mddi_host_reg_out(CMD, MDDI_CMD_LINK_ACTIVE);
	...
	for(cnt = 0; cnt<100; cnt++) {
		if(!(reg & 0x00000001)) { //check link is up
			reg = mddi_host_reg_in(STAT);
		}else {
			break;
		}
	...
	mutex_unlock(&Display_freeze_workaround);
}

int mddi_change_to_type_1(mddi_host_type host_idx)
{
	int reg = 0;

	if(!get_screen_after_boot())
		return -1;
...
void mddi_change_to_type_2(mddi_host_type host_idx)
{
	int reg, cnt;
...
void ext_mddi_SendGetClientStatus(void)
{	mddi_host_type host_idx = MDDI_HOST_EXT;

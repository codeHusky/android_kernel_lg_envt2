...
#include "mdp.h"
#include "mdp4.h"
#ifdef CONFIG_LGE_FB_MSM_MDDI_2 // BIGLAKE_Q_PATCH_LCD_SHIFT
#include <mach/clk.h>
...
// after setting up the factory reset flag, set first booting complete check
#ifdef CONFIG_LGE_DIAG_TESTMODE
extern void set_first_booting_complete_status(int status);
#endif
...
#endif
#endif /*CONFIG_LGE_BOOTLOADER_DISP_INIT*/
...
/* LG_FW : 2010.04.23 - jinho.jang  */
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH
static struct fb_var_screeninfo *last_var;
static struct fb_info *last_info;

#define BACKLIHGT_FORCE_ON	2

static struct msm_fb_data_type *local_mfd0; // to save first mfd
static int lcd_backlight_force_on_flag; // brightnessmode setting - whenever folder is changed and screen_frozen is called, this flag is set to 2
static int msm_fb_operation_mode = 1; // when phone goes to suspend mode, this flag is set to false, after resume is called , it is set to true
static int is_resume_mode = 0; // while resume process
static int is_suspend_mode = 0; // while suspend process
static int backlight_level = 0x0A; //default backlight current 9.0mA
static int folder_switch_event = 0; // while folder switch
static int is_screen_after_boot = 0;

static int mfd_ref_cnt = 0;

int get_screen_after_boot(void)
{
	return is_screen_after_boot;
}

static void is_boot_completed(struct led_classdev *led_cdev)
{
	if(led_cdev->screenAfterBoot == 1)
	{
		is_screen_after_boot = 1;

// move factory reset check here so that factory reset flag is set after boot completed
#if defined (CONFIG_LGE_APPS_FACTORY_RESET)
		if(factory_reset_checked == 0)
		{
			printk(KERN_INFO "%s, call factory_reset_check\n", __func__);
			factory_reset_check();
			factory_reset_checked = 1;
			printk(KERN_INFO "%s, end factory_reset_check\n", __func__);
		}
#endif /*CONFIG_LGE_APPS_FACTORY_RESET*/

// after setting up the factory reset flag, set first booting complete check
#ifdef CONFIG_LGE_DIAG_TESTMODE
		set_first_booting_complete_status(1);
#endif
	}
	else
	{
		is_screen_after_boot = 0;
// if the flag is not set, consider this as boot has not been completed
#ifdef CONFIG_LGE_DIAG_TESTMODE
		set_first_booting_complete_status(0);
#endif		
	}
}
#endif
					...
					enum led_brightness value)
{
	...
	lcd_backlight_force_on_flag = led_cdev->brightnessMode;
	backlight_level = bl_lvl;

	//if resume is running, then return
	if(!msm_fb_operation_mode)
		return;

	//check which fb is called	
	if(is_folder_open()){
		if(mfd->index == 0)
			mfd = mfd->mfd1;
	}
	
	//screen_frozen is called.
	//It means that folder is changed, and surfaceflihger finished fb update.
	if(lcd_backlight_force_on_flag == BACKLIHGT_FORCE_ON) {
		//If panel power is not on, We need to update just one frame which is in FB.
		//If panel power is on, already fb is updated with black image. We can just wait next frame.
	...
	//if panel is powered off, then return
	if(!mfd->panel_power_on)
		return;
#endif
...
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH
	.boot_mode_set = is_boot_completed,
#endif
};
#endif
...
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH	
	mfd->op_enable = 1;
	mfd->mfd1->op_enable = mfd->op_enable;
	mfd->mfd1->suspend.op_enable = mfd->mfd1->op_enable;
	
	if(is_folder_open()) { //open
		mfd = mfd->mfd1;
	}
#else
...
/* LG_FW : 2010.04.23 - jinho.jang	*/
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH	
	mfd->suspend.op_enable = 1;
	mfd->mfd1->suspend.op_enable = mfd->suspend.op_enable;
	
	// resume state var recover 
	mfd->mfd1->op_enable = mfd->mfd1->suspend.op_enable;
...
#ifdef CONFIG_LGE_FB_MSM_MDDI_2 // BIGLAKE_Q_PATCH_LCD_SHIFT
		pm_qos_update_requirement(PM_QOS_SYSTEM_BUS_FREQ , "mddi_ext",MSM_AXI_MAX_FREQ);
#endif
...
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH
	is_resume_mode = 0; // to confirm
	msm_fb_operation_mode = 1;
#endif	
}
#endif
...
/* LG_FW : 2010.04.23 - jinho.jang  */
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH
		if(folder_switch_event || is_resume_mode) // this is called from folder switch event or resume mode
		{
			u32 old_lvl;
			...
			if(bkl_lvl > 0) { // when power on
				if(!boot_animation && is_touch_initialzed()){
					if(mfd->index == 0) //fb 0 - sub lcd  touchscreen
						slcd_touch_on();
					if(mfd->index == 1) //fb 1 - main lcd touchscreen
						mlcd_touch_on();
				}
				folder_switch_event = 0;
			}
		}
		else // to set backlight brightness
		{
...
msm_fb_fill_curr_fb(void)
{
	//fill fb to previous image
	msm_fb_pan_display(last_var, last_info);
}

void msm_fb_gram_refresh(void)
{
	memset((void *)last_info->screen_base, BLACK, last_info->fix.smem_len);
	msm_fb_pan_display(last_var, last_info);
}
#endif /*CONFIG_LGE_DUAL_LCD_SWITCH*/
...
#ifdef CONFIG_LGE_FB_MSM_MDDI_2
	int cpu_khzfreq = 0;
#endif
...
/* LG_FW : 2010.04.23 - jinho.jang	*/
	...
	//folder switch - not suspend or resume mode
	if(op_enable && msm_fb_operation_mode && !boot_animation) {	
...
//check if AXI BUS is max freq or not
#ifdef CONFIG_LGE_FB_MSM_MDDI_2		
		 //check cpu frequency
		 //freq : 998400 , 768000, 576000, 384000
		cpu_khzfreq = cpufreq_quick_get(smp_processor_id());
		if(cpu_khzfreq < 998400) {
			pm_qos_update_requirement(PM_QOS_SYSTEM_BUS_FREQ , "mddi_ext",MSM_AXI_MAX_FREQ);			
			printk(KERN_ERR "msm_fb_blank_sub: cpu_khzfreq = %d !\n", cpu_khzfreq);
		}
#endif

// if folder switch is processing, mfd will be changed which one is selected between fb0 and fb1
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH
		if(!msm_fb_operation_mode) { // resume mode
			if (!mfd->panel_power_on) {
		...
		} else { // folder switch or fb open call
			if (!mfd->panel_power_on) {
				ret = pdata->on(mfd->pdev);
				if (ret == 0) {
					mfd->panel_power_on = TRUE;

					/* screen_frozen is set to 1, only when folder change state */					 
					//Before panel power is on, But when SCREEN_FROZEN is called, just fill current FB, and then turn on backlight
					if(!boot_animation) { //geneal folder switch						
						if(!get_screen_after_boot()) { //before boot completed - while charging or boot animation
							if(get_charging_disp()) // for battery charging display, fb update
								msm_fb_fill_curr_fb();
							else
								msm_fb_gram_refresh(); // for normal boot animation, fill fb to black
						} else {							
							if(mfd->screen_frozen) {
								msm_fb_fill_curr_fb();
								mfd->screen_frozen = 0;
							} else {
								msm_fb_gram_refresh();
...
/********************************************************************************/
	...
	default:
		...
		/* LG_FW : 2010.06.30 jinho.jang - touchscreen off before panel off */ 
		#ifdef CONFIG_LGE_DUAL_LCD_SWITCH
			if(!boot_animation && is_touch_initialzed()){
				if(mfd->index == 0) //fb 0 - sub lcd  touchscreen
					slcd_touch_off();
...
static int msm_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	if(blank_mode == FB_BLANK_POWERDOWN) {
		folder_switch_event = 1;
	}

	mfd->op_enable = 1;
	mfd->mfd1->op_enable = 1;
#endif
...
/* LG_FW : 2010.05.02 jinho.jang  */
#if defined(CONFIG_LGE_DUAL_LCD_SWITCH)
	if (mfd->index == 0) 
#endif
	{
...
#ifdef CONFIG_LGE_DUAL_LCD_SWITCH
	if (!local_mfd0) {
		local_mfd0 = mfd; // mfd = Pri MDDI
	} else {
		local_mfd0->mfd1 = mfd; // mfd1 = Ext	MDDI 
...
#ifdef CONFIG_LGE_BOOTLOADER_DISP_INIT
#ifdef CONFIG_FB_MSM_LOGO
	if(mfd_ref_cnt > 1 && msm_fb_get_console_inactive())
...
/* LG_FW: 2010.05.12 jinho.jang - initialization at bootloader */
	...
	if(boot_animation) {
		if(is_folder_open()) {			
			msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi, 0); //mddi panel on
			msm_fb_blank_sub(FB_BLANK_POWERDOWN, mfd->fbi, 0); //mddi panel off
			msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->mfd1->fbi, 0); //ext mddi panel on

		}else {			
			msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->mfd1->fbi, 0); //ext mddi panel on
			msm_fb_blank_sub(FB_BLANK_POWERDOWN, mfd->mfd1->fbi, 0); //ext mddi panel off
			msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi, 0); //mddi panel on
		}
	}
#endif
...
/* LG_FW : 2010.04.23 - jinho.jang  */
	...
	// check if correct lcd is selected
	if(msm_fb_operation_mode && !boot_animation && !folder_switch_event) {
		if(mfd->index == 0) {
			//check panel power state
			if(folder_is_opened) { //open
				if(mfd->mfd1->panel_power_on == 0) {
					msm_fb_blank_sub(FB_BLANK_POWERDOWN, mfd->fbi, 0); //mddi panel off
					msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->mfd1->fbi, 0); //ext mddi panel on
				}
			} else { //folder close state
				if(mfd->panel_power_on == 0) {
					msm_fb_blank_sub(FB_BLANK_POWERDOWN, mfd->mfd1->fbi, 0); //ext mddi panel off
					msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi, 0); //mddi panel on
				}
			}
...
/* LG_FW : 2010.04.23 - jinho.jang	*/
#ifdef CONFIG_LGE_BOOTLOADER_DISP_INIT
	//first when boot verizon animation is started
	if(boot_animation) {
		mfd->bl_level = backlight_level;
	...
	//when folder switch or resume  is called
	if(folder_switch_event || is_resume_mode) { // to confirm lcd black screen after power on by folder switch		
		mfd->bl_level = backlight_level;
		if(backlight_level == 0)
			mfd->bl_level = 0x0B;
		mdelay(100);
	...
	//to confirm check if right backlight is selected
	if(!boot_animation && msm_fb_operation_mode && !folder_switch_event) {
		if(folder_is_opened) { //open
			if(get_mlcd_backligit_status() == 0) {
				mfd->bl_level = backlight_level;		
				mdelay(100);
				msm_fb_set_backlight(mfd, mfd->bl_level, 0);
			}
		} else { //close
			if(get_slcd_backligit_status() == 0) {
				mfd->bl_level = backlight_level;
				mdelay(100);
				msm_fb_set_backlight(mfd, mfd->bl_level, 0);
			}
...
static int msm_fb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	case 32:
		...
		if ((var->blue.length != 8) ||
		    ...
		    (var->red.msb_right != 0))
/* LG_FW : 2010.05.02 jinho.jang - 24bit support */
#if 0//transp.offset has been removed on FC RELEASE
#if defined(CONFIG_LGE_FB_MSM_MDDI_LGIT_WVGA) //32bit
			((var->transp.offset == 24) &&
#else
			((var->transp.offset == 0) &&
...
#if defined CONFIG_FB_MSM_MDP31 //LGE_5035_PATCH - remove ||CONFIG_FB_MSM_MDP30
...
int mdp_blit(struct fb_info *info, struct mdp_blit_req *req)
{
	...
	return ret;
#elif defined CONFIG_FB_MSM_MDP30 //LGE_5035_PATCH
	/* MDP width split workaround */

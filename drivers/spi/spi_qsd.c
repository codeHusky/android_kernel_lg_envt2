...
#if 1//LGE_5035_PATCH - need to check, almost same to the AVAGO_SPI feature below but just setting dma_alignment is different
...
#else
#if defined(CONFIG_LGE_TOUCHSCREEN_AVAGO_SPI)
	if (pdata && pdata->dma_config) {
		rc = pdata->dma_config();
		if (!rc) {
			resource  = platform_get_resource_byname(pdev,
...
#endif /*CONFIG_LGE_TOUCHSCREEN_AVAGO_SPI*/
#endif

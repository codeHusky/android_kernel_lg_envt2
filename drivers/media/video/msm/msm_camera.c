...
#ifdef CONFIG_LGE_CAMERA_QCT_FREEZE_FIX
		...
		struct msm_stats_event_ctrl *se)
{
	...
	if (rc < 0) {
		pr_info("%s: msm_pmem_frame_ptov_lookup failed for y_phy=0x%x cbcr_phy=0x%x, data->phy.y_phy=0x%x,data->phy.cbcr_phy=0x%x\n",
			__func__, data->phy.y_phy, data->phy.cbcr_phy, data->phy.y_phy,data->phy.cbcr_phy);
		return rc;
	}
...
static int msm_get_stats(struct msm_sync *sync, void __user *arg)
{
	...
	case MSM_CAM_Q_VFE_MSG:
		...
		pr_info("%s: data->type %d qcmd->type %d length %d msd_id %d\n", __func__,
			data->type,
			qcmd->type,
			se.stats_event.len,
		...
		if ((data->type >= VFE_MSG_STATS_AEC) &&
			...
			if( (sync->pp_frame_avail == 1) &&
				...
				(data->type == VFE_MSG_OUTPUT_P) ) {
					CDBG("%s:%d for preiew post-processing!!!, pp_frame_avail=%d\n",
					__func__, __LINE__, sync->pp_frame_avail);
					rc = msm_divert_frame(sync, data, &se);
					sync->pp_frame_avail = 0;
			} else {
				if ((sync->pp_mask & PP_PREV) &&
					(data->type == VFE_MSG_OUTPUT_P)) {
					pr_info("%s:skip output indication\n", __func__);
					free_qcmd(qcmd);
					return 0;
				} else
			   ...
			   int once)
{
	...
	CDBG("%s: open %s, timestamp: %s\n", __func__, filep->f_path.dentry->d_name.name,__TIMESTAMP__);

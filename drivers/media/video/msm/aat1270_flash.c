...
{	
	...
	printk("aat1270_flash_led_set_current on = %d \n",on);
	
	/* LGE_CHANGE_S [jinsang.yun@lge.com] */
	switch(on){
		case 0:
			flash_led.brightness = LED_OFF;
			break;
		case 1:
			flash_led.brightness = LED_HALF;
			break;
		case 2:
			flash_led.brightness = LED_FULL;
			break;		
		}
	/* LGE_CHANGE_E [jinsang.yun@lge.com] */
	printk("aat1270_flash_led_set_current flash_led.brightness = %d \n",flash_led.brightness);
	...
	if (on == FLASH_ON || on == 2) {
		if (flash_led.brightness == LED_FULL) {
			...
			for(s2c_count = 0; s2c_count < 7; ++s2c_count) {
				printk("aat1270_flash_led_set_current, torch cnt(%d)\n", s2c_count);

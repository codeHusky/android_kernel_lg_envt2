...
static boolean vfe_send_bus_overflow_msg(struct msm_vfe_resp *rp,
			struct vfe_message *msg, void *data);
#ifdef CONFIG_LGE_CAMERA_SOF_OFF 
 //SOF Off
#else
	...
	uint8_t fragmentCount)
{
	...
	if (in->hwCurrentFlag == ping) {
		CDBG("%s: [pong] nextFragmentAddr = 0x%x, in->hwRegPongAddress = 0x%x\n", __func__, nextFragmentAddr, in->hwRegPingAddress);
		writel(nextFragmentAddr, in->hwRegPingAddress);
		in->hwCurrentFlag = pong;

	} else {
		CDBG("%s: [ping] nextFragmentAddr = 0x%x, in->hwRegPingAddress = 0x%x\n", __func__, nextFragmentAddr, in->hwRegPingAddress);
...
#ifdef CONFIG_LGE_CAMERA_SOF_OFF //2010.10.03 woochang.chun, added
	ctrl->vfeImaskLocal.camifSofIrq  = FALSE;
#else

/* drivers/input/touchscreen/slcd_touch_spi_amri5k.c
 *
...
#include "slcd_touch_amri5k.h"

/* LG_FW : 2010.03.29 jiho.jang - Dual LCD Switch */
#if defined(CONFIG_LGE_TOUCHSCREEN_AVAGO)
#include <mach/board.h>

static int is_slcd_touch_standby_mode = 0;
static int is_ts_initialized = 0;
#endif
...
static struct early_suspend s_ts_early_suspend;
static void amri5k_s_ts_early_suspend(struct early_suspend *h);
static void amri5k_s_ts_late_resume(struct early_suspend *h);
#endif

#undef DEBUG_PRINT	// Define only for debug mode
//#define DEBUG_PRINT
#ifdef DEBUG_PRINT
	#define dprintk(f...)	printk(KERN_ERR "AMRI5K_sub_touch " f)
#else
...
#define S_TS_DRIVER_NAME "amri5k_s_ts"
...
#define AMRI5K_CALIBRATION_SCAN_FREQUENCE   //2010/09/09
...
#define SUB_TOUCH_RESET			33
#define SLCD_AMRI5K_IRQ_GPIO	42
...
#define THUMB_AREA_SIZE         150
#define THUMB_REJECT_AREA       80
#endif
...
#define S_AMRI5K_OSC_CLK      0x03
#define S_AMRI5K_HB           0x05
#ifdef AMRI5K_FIRST_INT_DISABLE
static int s_ignore_first_intterupt_flg = FALSE;
#endif
#ifdef AMRI5K_ESD_RECOVERY
static int s_touch_data_read_status = FALSE;
#endif
/*------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------*/
// type define
...
struct AMRI5K_s_ts_device {
	...
	struct mutex		lock;
#ifdef AMRI5K_SHUTDOWN_TOUCH_AFTER_POWER_ON    
    struct delayed_work touchoffwork;
#endif
#ifdef AMRI5K_ESD_RECOVERY
...
extern int AMRI5K_Mlcd_InitSromDownload(void);
extern u8 AMRI5K_m_spi_read(u8 reg);
extern u8 AMRI5K_m_spi_write(unsigned int reg, unsigned int val);
#endif

static struct input_dev *AMRI5K_s_ts_input = NULL;
static struct AMRI5K_s_ts_device AMRI5K_s_ts_dev;
static struct spi_device *AMRI5K_s_ts_spi = NULL;

/*------------------------------------------------------------------------------------*/
extern void mddi_remote_lgit_lcd_off(void);
extern void ext_mddi_remote_lgit_lcd_off(void);
/*------------------------------------------------------------------------------------*/
// global variable
static struct workqueue_struct *ts_work_queue;
#ifdef AMRI5K_NOISE_PROCESSING
static u8 s_init_delta_done = 0;
static u8 s_usb_state = 0;
#endif

static unsigned char slcdTouchInitReg[] = {
//[Initialization Start ] -----------------------------------------------------------------------------
//[Touch Threshold Start ] ----------------------------------------------------------------------------
    0x0F,0x7A,          // Auto calibration enable ?
    ...
    0x47,126,
	0x40,0x26/*0x2F*//*0x26*//*0x30*/,  // DMOD_CLOCK(Sub Touch)
//[Initialization End ] -------------------------------------------------------------------------------
//[Resolution Start ] ---------------------------------------------------------------------------------
...
//[Adjust frame rate Start] ---------------------------------------------------------------------------
    0x23,S_AMRI5K_OSC_CLK,// OSC_MODE #1 internal clock speed 24M (0x03) -> 33.5M     // Test 20100802
    0x22,S_AMRI5K_HB,   // Line Scan rate -  0A -> 05 30Hz -> 60Hz 
...
//[Touch Threshold Start ] ----------------------------------------------------------------------------	
    0x67,0x02/*0x04*//*0x00*/,          // Max Touch change by avago.2010.09.30
    0x68,0x58/*0xFA*/,          // Max Touch
	0x5A,0, 			// TOUCH_1_HIGH 
	0x5B,120,	        // TOUCH_1_LOW
#ifndef AMRI5K_NOISE_PROCESSING
	0x5C,1, 			// TOUCH_2_HIGH
	0x5D,0x2C,          //200/*150*/,		    // TOUCH_2_LOW
	0x5E,0x01, 			// TOUCH_3_HIGH
	0x5F,0x5E,          //250/*210*/,           // TOUCH_3_LOW
#endif
//[Touch Threshold End ] ------------------------------------------------------------------------------
//    0x75,0x09,          // NAV Filter // 0x08 -> 0x09 NV0 set. NV0: Increase panel sampling. NV1:Use higher than normal output data averaging
    ...
    0x0C,0x05,          // Flipping XY	-Sub lcd touch
...
#ifdef AMRI5K_NOISE_PROCESSING         // 2010/09/15
...
void AMRI5K_Slcd_Touch_HwReset(void);
#endif

int AMRI5K_Slcd_SROM_Download(void);
int AMRI5K_Slcd_InitSromDownload(void);

/*------------------------------------------------------------------------------------*/
// Fuction list
/* Read-only message with current device setup */
static u8 AMRI5K_s_ts_spi_read_reg(u8 reg)
{
	int ret;
	u8 val;
	u8 code = reg;

	ret = spi_write(AMRI5K_s_ts_spi,&code,1);
	...
	ret = spi_read(AMRI5K_s_ts_spi,&val, 1);
	if (ret < 0) {
		printk(KERN_ERR "spi read error reg:0x%X return:%d", reg, ret);
		return ret;
	}
...
static u8 AMRI5K_s_ts_spi_write_reg(unsigned int reg, unsigned int val)
{
	int ret_val;
	unsigned char buf[2];
	...
	ret_val = spi_write(AMRI5K_s_ts_spi,buf, 2);
	if(ret_val < 0)
	{
...
#ifdef AMRI5K_ESD_RECOVERY_POWER
u8 AMRI5K_s_spi_read(u8 reg)
{
    return AMRI5K_s_ts_spi_read_reg(reg);
}

u8 AMRI5K_s_spi_write(unsigned int reg, unsigned int val)
{
    return AMRI5K_s_ts_spi_write_reg(reg, val);
}
#endif

#ifdef AMRI5K_NOISE_PROCESSING
static void AMRI5K_s_ts_usb_setting(u8 plug)
{
    if(plug) // usb connected
    {
        dprintk("usb on... \n");
        AMRI5K_s_ts_spi_write_reg(0x5C, 0x00);  // TOUCH_2_HIGH
        AMRI5K_s_ts_spi_write_reg(0x5D, 0xFA);  // TOUCH_2_LOW , T2 0x118 -> 0x0FA
        AMRI5K_s_ts_spi_write_reg(0x5E, 0x01);  // TOUCH_3_HIGH
        AMRI5K_s_ts_spi_write_reg(0x5F, 0x20);  // TOUCH_3_LOW , T2 0x140 -> 0x120
        AMRI5K_s_ts_spi_write_reg(0x49, 0x40);  // 0x40
        AMRI5K_s_ts_spi_write_reg(0x78, 0x90);
//        AMRI5K_s_ts_spi_write_reg(0x2B, 0x06);
        AMRI5K_s_ts_spi_write_reg(0x4F, 0x01);
    }
    else
    {
        dprintk("usb off... \n");    
        AMRI5K_s_ts_spi_write_reg(0x5C, 0x00);  // TOUCH_2_HIGH
        AMRI5K_s_ts_spi_write_reg(0x5D, 200);   // TOUCH_2_LOW
        AMRI5K_s_ts_spi_write_reg(0x5E, 0x00);  // TOUCH_3_HIGH
        AMRI5K_s_ts_spi_write_reg(0x5F, 250);   // TOUCH_3_LOW
        AMRI5K_s_ts_spi_write_reg(0x49, 0x0A);
        AMRI5K_s_ts_spi_write_reg(0x78, 0x00);
//        AMRI5K_s_ts_spi_write_reg(0x2B, 0x03);
        AMRI5K_s_ts_spi_write_reg(0x4F, 0x01);
    }
}

void AMRI5K_s_ts_init_delta(u8 plug)
{
    s_usb_state = plug;
    dprintk("usb status:%d\n", plug);
    
    if(is_slcd_touch_standby_mode)
        s_init_delta_done = 0;
    else
    {
        AMRI5K_s_ts_usb_setting(plug);
        s_init_delta_done = 1;
    }
}
#endif
...
int AMRI5K_s_ts_ExtendVirtualCordinate(struct touch_data_format *ext_touch, struct amri5k_point *old_touch)
{
...
#ifdef AMRI5K_SHUTDOWN_TOUCH_AFTER_POWER_ON
static void AMRI5K_ts_shutdown_touch_chip_work(struct work_struct *work)
{
    dprintk("-----------------------------------------------------------\n");
	if(is_folder_open())
	{
        printk(KERN_ERR"AMRI5K Sub Touch & Lcd Off");
		slcd_touch_off();
		mddi_remote_lgit_lcd_off();
	}
	else
	{
        printk(KERN_ERR"AMRI5K Main Touch & Lcd Off");
		mlcd_touch_off();
		ext_mddi_remote_lgit_lcd_off();		
	}
...
#ifdef AMRI5K_ESD_RECOVERY
static void AMRI5K_s_ts_esd_recovery_work(struct work_struct *work)
{
    struct AMRI5K_s_ts_device *priv = container_of(work, struct AMRI5K_s_ts_device, esdwork.work);
    ...
    if((!is_folder_open()) && (is_slcd_touch_standby_mode == FALSE) && (s_touch_data_read_status == FALSE))
    {
#ifdef AMRI5K_ESD_RECOVERY_POWER    // Just debug
        dprintk("Main SROM Version 0x%02X\n", AMRI5K_m_spi_read(0x01));
#endif
        regData = AMRI5K_s_ts_spi_read_reg(0x01);
        ...
        if(regData != AMRI5K_SROM_VERSION)
        {
            ...
            {       
                ...
                mdelay(100);
                touch_power_reset = 1;
                if(AMRI5K_Slcd_InitSromDownload() < 0)
                {
                ...
                {
#ifdef AMRI5K_CALIBRATION_SCAN_FREQUENCE
                    AMRI5K_s_ts_spi_write_reg(0x71, priv->cal_clock[priv->cal_clock_sel]);
                    AMRI5K_s_ts_spi_write_reg(0x70, 0x36);
                    mdelay(10);
                    AMRI5K_s_ts_spi_write_reg(0x0F, 0x7B);
                    mdelay(10);                
#endif
            ...
            if(touch_power_reset)
            {
                ...
                {
                    dprintk("Main Touch Init!!\n");
                }
                AMRI5K_m_spi_write(0x7a,0xaa); //Standby mode
                dprintk("Main Touch standby mode set\n");
            }
#endif
        }


#if 0 // #ifdef AMRI5K_NOISE_PROCESSING   // noise data collection.

        dprintk("Data collection!!\n");

        regData = AMRI5K_s_ts_spi_read_reg(0x61);
        ttt = regData;
        regData = AMRI5K_s_ts_spi_read_reg(0x60);
        ttt = (regData<<8)|ttt;
        dprintk("sum9cells value= %x\n",ttt);
        regData = AMRI5K_s_ts_spi_read_reg(0x78);
        dprintk("COMMAND_CONTROL = %x\n",regData);
//    AMRI5K_s_ts_spi_write_reg(0x70,0x36);
#endif
    }

    //    schedule_delayed_work(&priv->esdwork, msecs_to_jiffies(AMRI5K_ESD_POLLING_TIME));
    queue_delayed_work(ts_work_queue, &priv->esdwork, msecs_to_jiffies(AMRI5K_ESD_POLLING_TIME));
}
#endif
...
/*------------------------------------------------------------------------------------*/
static int AMRI5K_s_ts_ReadXYPosition(struct AMRI5K_s_ts_device *priv, u16 npoint)
{
	int i;
	u8  read_buf[0x26]={0,};
    ...
    static int real_release_check = OFF;
#endif
...
#ifdef AMRI5K_ESD_RECOVERY
    s_touch_data_read_status = TRUE;
#endif

#ifdef AMRI5K_ESD_RECOVERY
    ret_value = AMRI5K_s_ts_spi_read_reg(0x01);
    ...
    ret_value = AMRI5K_s_ts_spi_read_reg(0x02);
    //dprintk("---------------------------------------------------------\n");
    ...
    if((ret_value & 0x12) == 0x12)
    {
...
#endif
#ifdef AMRI5K_REAL_RELEASE_CHECK
        real_release_check = OFF;
#endif

        // Data Read mode
        AMRI5K_s_ts_spi_write_reg(0x7b,0x01);
        // Read Touch data
        ret_value = AMRI5K_s_ts_spi_read_reg(0x01);
        ...
        if(priv->touchdata.nbpoints >= 1)
        {
			for(i=0; i < 9; i++)
            {
                ret_value = AMRI5K_s_ts_spi_read_reg(0x02+i);
        ...
        if(priv->touchdata.nbpoints >= 2)
        {
			for(i=0; i < 9; i++)
            {
                ret_value = AMRI5K_s_ts_spi_read_reg(0x0B+i);
				read_buf[i] = ret_value;
            }
        ...
        // Instruction Mode
        AMRI5K_s_ts_spi_write_reg(0x7b,0x00);
        ...
        for(i=0; i<priv->touchdata.nbpoints; i++)
        {
			...
			dprintk("TP:%d Real:0x%X Touch ID:0x%X X:%d Y:%d Force:%d Area:%d T Cnt:%d\n", 
                ...
                if(priv->touchdata.nbpoints == 1)
                {
                    if((priv->touchdata.tpd[0].force >= 650)&& (thumbs_filter_flag == OFF))
                    {
                        thumbs_filter_flag = ON;
                        dprintk("Thumb Mode Set\n");
                        
                        AMRI5K_s_ts_spi_write_reg(0x58,0xB5);
                        AMRI5K_s_ts_spi_write_reg(0x15,0x08);
#ifdef AMRI5K_NOISE_PROCESSING
                        if(s_usb_state == 0)    // usb disconnect
                        {
                            AMRI5K_s_ts_spi_write_reg(0x49,0x1E);   // 2010.08.31 0x1A->0x1E
//                            AMRI5K_s_ts_spi_write_reg(0x2B,0x04);   // 2010.08.31 add
                        }                                
#else
                        AMRI5K_s_ts_spi_write_reg(0x49,0x1E);   // 2010.08.31 0x1A->0x1E
//                        AMRI5K_s_ts_spi_write_reg(0x2B,0x04);   // 2010.08.31 add
        ...
        if(priv->touchdata.nbpoints == 2)
        {
                ...
                && (thumbs_filter_flag == OFF))
            {
                thumbs_filter_flag = ON;
                dprintk("Thumb Mode Set\n");
                //AMRI5K_s_ts_spi_write_reg(0x1A,0x09);        // 2010.08.31 
                //AMRI5K_s_ts_spi_write_reg(0x27,0x02);        // 2010.08.31 
...
//                AMRI5K_s_ts_spi_write_reg(0x2B,0x04);   // 2010.08.31 add
...
#ifdef AMRI5K_IGNORE_EDGE_TOUCH
    if(priv->touchdata.nbpoints == 1)
    {
        if(  (priv->touchdata.tpd[0].x <= 5) || (priv->touchdata.tpd[0].x >= AMRI5K_X_MAX - 5)
          || (priv->touchdata.tpd[0].y <= 5) || (priv->touchdata.tpd[0].y >= AMRI5K_Y_MAX - 5))
        {
            dprintk("Ingore Edge Touch [Single case] !!\n");
            goto err_return;
        }
    }
    else if(priv->touchdata.nbpoints == 2)
    {
         if(  (priv->touchdata.tpd[0].x <= 5) || (priv->touchdata.tpd[0].x >= AMRI5K_X_MAX - 5)
           || (priv->touchdata.tpd[0].y <= 5) || (priv->touchdata.tpd[0].y >= AMRI5K_Y_MAX - 5))
        {
           priv->touchdata.nbpoints = 1;
           priv->touchdata.tpd[0].touchid   = priv->touchdata.tpd[1].touchid;
           priv->touchdata.tpd[0].x         = priv->touchdata.tpd[1].x;
           priv->touchdata.tpd[0].y         = priv->touchdata.tpd[1].y;
           priv->touchdata.tpd[0].area      = priv->touchdata.tpd[1].area;
           priv->touchdata.tpd[0].force     = priv->touchdata.tpd[1].force;
           dprintk("Ingore Edge Touch [Multi case] !!\n");
        }
        else if(  (priv->touchdata.tpd[1].x <= 5) || (priv->touchdata.tpd[1].x >= AMRI5K_X_MAX - 5)
               || (priv->touchdata.tpd[1].y <= 5) || (priv->touchdata.tpd[1].y >= AMRI5K_Y_MAX - 5))
        {
           priv->touchdata.nbpoints = 1;
        }
    }
...
// 2010.09.23 The code delete by avago
/*        if((50 <= priv->touchdata.tpd[0].area) && (priv->touchdata.tpd[0].area <= 200))
        {
            if((priv->touchdata.tpd[0].area % 10) == 0)
            {
                if((single_touch_keydown == FALSE) && (multi_touch_release == FALSE))
                {
                    dprintk("ESD Recovery multiple of 10 !! %d\n", priv->touchdata.tpd[0].area);
                    goto err_return;
                }
            }
        }
*/
    ...
    else if((ret_value & 0x14) == 0x14)
    {
        ...
        ret_value = queue_delayed_work(ts_work_queue, &priv->releasework, msecs_to_jiffies(TS_RELEASE_TIME));
        ...
        if(thumbs_filter_flag == ON)
        {
            ...
            thumbs_filter_flag = OFF;
            AMRI5K_s_ts_spi_write_reg(0x58,0x35);
            AMRI5K_s_ts_spi_write_reg(0x15,0x04);
#ifdef AMRI5K_NOISE_PROCESSING
            if(s_usb_state == 0)
            {
//                AMRI5K_s_ts_spi_write_reg(0x2B,0x03);   // 2010.08.31   add
                AMRI5K_s_ts_spi_write_reg(0x49,0x0A);   // 2010.08.31   add
            }
#else
//            AMRI5K_s_ts_spi_write_reg(0x2B,0x03);   // 2010.08.31   add
            AMRI5K_s_ts_spi_write_reg(0x49,0x0A);   // 2010.08.31   add
    ...
    {
#ifdef AMRI5K_REAL_RELEASE_CHECK
        if (real_release_check == ON)
        {
            real_release_check = OFF;
            dprintk("real_release !!\n");
        }
        else
        {
            dprintk("non_real_release !!\n");
            goto err_return;
        }
#endif
...
#ifdef AMRI5K_ESD_RECOVERY
    s_touch_data_read_status = FALSE;
#endif
#ifdef AMRI5K_DELAY_RELEASE_TIME
    return 0;
...
static void AMRI5K_s_ts_read_touch_event_work(struct work_struct *work)
{
    struct AMRI5K_s_ts_device *priv = container_of(work, struct AMRI5K_s_ts_device, poswork);
...
#ifdef AMRI5K_FIRST_INT_DISABLE
    if(s_ignore_first_intterupt_flg == TRUE)
    {
        s_ignore_first_intterupt_flg = FALSE;
	...
	error = AMRI5K_s_ts_ReadXYPosition(priv, priv->touchmode);
	if(error < 0)
	{
...
#else
	AMRI5K_s_ts_ReadXYPosition(priv->client, 1);
#endif
	if(tdata->nbpoints > 0)
	{
	...
	else if(priv->key_eType == KEY_RELEASE_EVENT)
	{
        ...
        error = AMRI5K_s_ts_ExtendVirtualCordinate(&ext_touch, old_touch);
...
#ifdef AMRI5K_DELAY_RELEASE_TIME
static void AMRI5K_s_ts_release_touch_event_work(struct work_struct *work)
{
    struct AMRI5K_s_ts_device *priv = container_of(work, struct AMRI5K_s_ts_device, releasework.work);
    dprintk("run release workqueue. status:%d\n", priv->rel_status);
    priv->rel_status = REL_WORKQUEUE_EMPTY;
    AMRI5K_s_ts_read_touch_event_work(&priv->poswork);
}
#endif
...
{
	/* Sub LCD Touchscreen */
	gpio_configure(SUB_TOUCH_RESET, GPIOF_DRIVE_OUTPUT); 
	gpio_set_value(SUB_TOUCH_RESET, 1);
	mdelay(10);
	gpio_set_value(SUB_TOUCH_RESET, 0);
	mdelay(10);
	gpio_set_value(SUB_TOUCH_RESET, 1);
	mdelay(100);
}
...
int AMRI5K_Slcd_SROM_Download(void)
{
    ...
    // Disable Watchdog
    AMRI5K_s_ts_spi_write_reg(0x7d,0xad);
    // Clear BOOT_STATE
    AMRI5K_s_ts_spi_write_reg(0x0e,0x00);
    // Enable download 
    AMRI5K_s_ts_spi_write_reg(0x08,0x2b);
    // Download patchcode
    cnt = 0;
    do{
        AMRI5K_s_ts_spi_write_reg(0x09,amri5k_slcd_srom_code[cnt]);
        udelay(15);     // Device need to wait time during patch download.
        cnt++;
    }while(cnt < sizeof(amri5k_slcd_srom_code));
    ...
    cnt = 0;
    do{
        ret_value = AMRI5K_s_ts_spi_read_reg(0x0e);
    ...
    // Enable watchdog
    AMRI5K_s_ts_spi_write_reg(0x7d,0x00);
...
int AMRI5K_Slcd_InitSromDownload(void)
{
	...
	mutex_lock(&AMRI5K_s_ts_dev.lock);

    do
    {
        AMRI5K_Slcd_Touch_HwReset();

        // Software Reset
        AMRI5K_s_ts_spi_write_reg(0x7a,0xaa);
        AMRI5K_s_ts_spi_write_reg(0x7a,0xbb);
        mdelay(20);

        // Read Device ID 
        ret_value = AMRI5K_s_ts_spi_read_reg(0x00);
        ...
        ret_value = AMRI5K_Slcd_SROM_Download();
        if(ret_value < 0)
        {
    ...
    for(bootCnt=0; bootCnt < sizeof(slcdTouchInitReg); bootCnt += 2)
    {
        AMRI5K_s_ts_spi_write_reg(slcdTouchInitReg[bootCnt],slcdTouchInitReg[bootCnt+1]);
        dprintk("REG: 0x%02X 0x%02X\n",slcdTouchInitReg[bootCnt],slcdTouchInitReg[bootCnt+1]);
    }

#ifdef AMRI5K_NOISE_PROCESSING
    AMRI5K_s_ts_usb_setting(s_usb_state);
#endif

	ret_value = AMRI5K_s_ts_spi_read_reg(0x02);	
    dprintk("Register Initialize End Status:0x%X \n",ret_value);

	mutex_unlock(&AMRI5K_s_ts_dev.lock);
	return 0;
...
{
#ifdef CONFIG_LGE_MULTITOUCH
  input_report_abs(AMRI5K_s_ts_input, ABS_MT_TOUCH_MAJOR, 1);
  input_report_abs(AMRI5K_s_ts_input, ABS_MT_POSITION_X, x);
  input_report_abs(AMRI5K_s_ts_input, ABS_MT_POSITION_Y, y);
  input_mt_sync(AMRI5K_s_ts_input);
  input_sync(AMRI5K_s_ts_input);

  input_report_abs(AMRI5K_s_ts_input, ABS_MT_TOUCH_MAJOR, 0);
  input_report_abs(AMRI5K_s_ts_input, ABS_MT_POSITION_X, x);
  input_report_abs(AMRI5K_s_ts_input, ABS_MT_POSITION_Y, y);
  input_mt_sync(AMRI5K_s_ts_input);
  input_sync(AMRI5K_s_ts_input);
#else
  input_report_abs(AMRI5K_s_ts_input, ABS_X, x);
  input_report_abs(AMRI5K_s_ts_input, ABS_Y, y);  
  input_report_key(AMRI5K_s_ts_input, BTN_TOUCH, 0);
  input_sync(AMRI5K_s_ts_input);
#endif
}

EXPORT_SYMBOL(Send_Touch);
static irqreturn_t AMRI5K_s_ts_isr(int irq, void *dev_id)
{
	struct AMRI5K_s_ts_device *priv = dev_id;
#ifdef TOUCH_DELY_TEST
	schedule_delayed_work(&priv->poswork, msecs_to_jiffies(TS_POLLING_TIME));
#else
	queue_work(ts_work_queue, &priv->poswork);
#endif
	return IRQ_HANDLED;
}

static int AMRI5K_s_ts_open(struct input_dev *dev)
{	
	return 0;
}

static void AMRI5K_s_ts_close(struct input_dev *dev)
{
	return;
}
...
void slcd_touch_on(void)
{
	...
	if(&AMRI5K_s_ts_dev == NULL || AMRI5K_s_ts_input == NULL)
		return;
	...
	if(is_slcd_touch_standby_mode)
	{
...
#endif
		AMRI5K_s_ts_spi_write_reg(0x7a,0xdd); // Resume
        mdelay(20); //mdelay(10);
#ifdef AMRI5K_ESD_RECOVERY_POWER                
        regData = AMRI5K_s_ts_spi_read_reg(0x01);
        if(regData != AMRI5K_SROM_VERSION)
        ...
        {
#ifdef AMRI5K_REWIRTE_REGISTER
            for(regCnt=0; regCnt < sizeof(slcdTouchInitReg); regCnt+=2)
            {
                if(slcdTouchInitReg[regCnt] == 0x0F)    // Pass 0x0F register
                {
                    continue;
                }
                
                regData = AMRI5K_s_ts_spi_read_reg(slcdTouchInitReg[regCnt]);
                if(regData != slcdTouchInitReg[regCnt+1])
                {   
                    AMRI5K_s_ts_spi_write_reg(slcdTouchInitReg[regCnt], slcdTouchInitReg[regCnt+1]);
                    dprintk("Rewrite REG: 0x%02X 0x%02X Real: 0x%02X\n",slcdTouchInitReg[regCnt],slcdTouchInitReg[regCnt+1], regData);
                }
            }
#endif
        }
...
#ifdef AMRI5K_CALIBRATION_SCAN_FREQUENCE
        AMRI5K_s_ts_dev.cal_clock_sel = 0;
        dprintk( "0x71 = 0x%x\n", AMRI5K_s_ts_dev.cal_clock[AMRI5K_s_ts_dev.cal_clock_sel]);
        AMRI5K_s_ts_spi_write_reg(0x71, AMRI5K_s_ts_dev.cal_clock[AMRI5K_s_ts_dev.cal_clock_sel]);
        AMRI5K_s_ts_spi_write_reg(0x70, 0x36);
        mdelay(10);
        AMRI5K_s_ts_spi_write_reg(0x0F, 0x7B);
        mdelay(30);
		...
		regData = AMRI5K_s_ts_spi_read_reg(0x39);
		if (regData == 0x00)
		{
			mdelay(50);
			AMRI5K_s_ts_spi_write_reg(0x39,0x15);
			AMRI5K_s_ts_spi_write_reg(0x75,0x10);
			mdelay(10);
		}
#endif
	...
	while(1)
	{
		data1 = AMRI5K_s_ts_spi_read_reg( 0x0F);
	...
	AMRI5K_s_ts_spi_write_reg(0x71, 0x60);
	AMRI5K_s_ts_spi_write_reg(0x72, 0x08);
	AMRI5K_s_ts_spi_write_reg(0x70, 0x33);
	mdelay(30);
#endif
#ifdef AMRI5K_NOISE_PROCESSING
        if(s_init_delta_done == 0)
        {
            AMRI5K_s_ts_usb_setting(s_usb_state);
        }
#endif
...
#ifdef AMRI5K_FIRST_INT_DISABLE
        s_ignore_first_intterupt_flg = TRUE;
#endif    
        enable_irq(AMRI5K_s_ts_dev.irq);
        mutex_unlock(&AMRI5K_s_ts_dev.lock);
        is_slcd_touch_standby_mode = FALSE;
        dprintk("slcd resume & int enable \n");	
	}	
			
	dprintk("slcd touch on \n");
}


#ifdef AMRI5K_REGISTER_DUMP
static void slcd_touch_register_dump(void)
{
    int addrCnt = 0;
    ...
    {
        retValue = AMRI5K_s_ts_spi_read_reg(addrCnt);
...
void slcd_touch_off(void)
{
	...
	if(!is_slcd_touch_standby_mode)
	{
		disable_irq(AMRI5K_s_ts_dev.irq);

		mutex_lock(&AMRI5K_s_ts_dev.lock);		

#ifdef AMRI5K_REGISTER_DUMP
        slcd_touch_register_dump();
#endif
        is_slcd_touch_standby_mode = TRUE;
		AMRI5K_s_ts_spi_write_reg(0x7a,0xaa); //Standby mode
...
#endif
		mutex_unlock(&AMRI5K_s_ts_dev.lock);
		dprintk("slcd shutdown & int disable \n");	
	}
		
	dprintk("slcd touch off \n");
}
#endif
...
int is_touch_initialzed(void)
{
	return is_ts_initialized;
}

static int AMRI5K_s_ts_probe(struct spi_device *spi)
{
	struct AMRI5K_s_ts_device *ts_dev;
	int error;
	int irq;	
#ifdef AMRI5K_CALIBRATION_SCAN_FREQUENCE
	...
	ts_dev = &AMRI5K_s_ts_dev;	
	AMRI5K_s_ts_spi = spi;
	
	dev_set_drvdata(&spi->dev, ts_dev);

	AMRI5K_s_ts_input = input_allocate_device();
	if (!AMRI5K_s_ts_input) {
		error = -ENOMEM;
		goto err1;
	}
	...
	AMRI5K_s_ts_input->name = S_TS_DRIVER_NAME;
	AMRI5K_s_ts_input->open = AMRI5K_s_ts_open;
	AMRI5K_s_ts_input->close = AMRI5K_s_ts_close;		
	
	set_bit(EV_SYN, 	 AMRI5K_s_ts_input->evbit);
	set_bit(EV_KEY, 	 AMRI5K_s_ts_input->evbit);
	set_bit(EV_ABS, 	 AMRI5K_s_ts_input->evbit);
	
#ifdef CONFIG_LGE_AUDIO_HAPTIC_TOUCH_SOFT_KEY						
    set_bit(TOUCH_BACK, AMRI5K_s_ts_input->keybit);
	set_bit(TOUCH_SEARCH, AMRI5K_s_ts_input->keybit);
#else
    set_bit(KEY_BACK, AMRI5K_s_ts_input->keybit);
    set_bit(KEY_SEARCH, AMRI5K_s_ts_input->keybit);
#endif	

#ifdef CONFIG_LGE_MULTITOUCH
	set_bit(ABS_MT_TOUCH_MAJOR, AMRI5K_s_ts_input->absbit);
	input_set_abs_params(AMRI5K_s_ts_input, ABS_MT_POSITION_X, 0, AMRI5K_X_MAX, 0, 0);
	input_set_abs_params(AMRI5K_s_ts_input, ABS_MT_POSITION_Y, 0, AMRI5K_Y_MAX, 0, 0);
	input_set_abs_params(AMRI5K_s_ts_input, ABS_MT_TRACKING_ID, 0, 5, 0, 0);
	input_set_abs_params(AMRI5K_s_ts_input, ABS_MT_TOUCH_MAJOR, 0, AMRI5K_FORCE_MAX, 0, 0);
	input_set_abs_params(AMRI5K_s_ts_input, ABS_MT_WIDTH_MAJOR, 0, AMRI5K_AREA_MAX, 0, 0);	
#else
	set_bit(BTN_TOUCH, AMRI5K_s_ts_input->keybit);
	input_set_abs_params(AMRI5K_s_ts_input, ABS_X, 0, AMRI5K_X_MAX, 0, 0);
	input_set_abs_params(AMRI5K_s_ts_input, ABS_Y, 0, AMRI5K_Y_MAX, 0, 0);
#endif	
	error = input_register_device(AMRI5K_s_ts_input);
	if (error) {
	...
	ts_dev->spi = spi;
	ts_dev->tdev = AMRI5K_s_ts_input;
    INIT_WORK(&ts_dev->poswork, AMRI5K_s_ts_read_touch_event_work);
#ifdef AMRI5K_SHUTDOWN_TOUCH_AFTER_POWER_ON
    INIT_DELAYED_WORK(&ts_dev->touchoffwork, AMRI5K_ts_shutdown_touch_chip_work);     
#endif
#ifdef AMRI5K_ESD_RECOVERY
    INIT_DELAYED_WORK(&ts_dev->esdwork, AMRI5K_s_ts_esd_recovery_work); 
#endif
#ifdef AMRI5K_DELAY_RELEASE_TIME
    INIT_DELAYED_WORK(&ts_dev->releasework, AMRI5K_s_ts_release_touch_event_work); 
#endif

	error = AMRI5K_Slcd_InitSromDownload();
	if(error)
	{		
		printk(KERN_ERR"failed to initial SLCD_TOUCH_AMRI5K\n");
		goto err1;	
	}
...
#ifdef AMRI5K_CALIBRATION_SCAN_FREQUENCE
    mdelay(100);
	data_old = AMRI5K_s_ts_spi_read_reg( 0x35);
	dprintk( "data_old:%d\n",data_old);
    ...
    mdelay(100);
	data1 = AMRI5K_s_ts_spi_read_reg( 0x35);
    ...
    osc_temp = (8192*S_AMRI5K_HB*delta)/msec_time;
    ...
    else
    {
        ...
        //m_clk =  osc_temp/234 - 82;          // 2*117KHz
    ...
    AMRI5K_s_ts_spi_write_reg(0x71, ts_dev->cal_clock[ts_dev->cal_clock_sel]);
    AMRI5K_s_ts_spi_write_reg(0x70, 0x36);
    mdelay(10);
    AMRI5K_s_ts_spi_write_reg(0x0F, 0x7B);
    mdelay(10);
    ...
    s_ignore_first_intterupt_flg = TRUE;
#endif    
	/* SLCD Touch Interrupt */
	/* Touch interrupt setting - sub lcd touch default */
	error = gpio_request(SLCD_AMRI5K_IRQ_GPIO, "amri5k_s_ts_irq");
	if (error) {
		printk(KERN_ERR"Unable to request touchscreen GPIO.\n");
		goto err2;
	}
	
	gpio_direction_input(SLCD_AMRI5K_IRQ_GPIO);	
	
	irq = gpio_to_irq(SLCD_AMRI5K_IRQ_GPIO);
	if (irq < 0) {
		error = irq;
		printk(KERN_ERR"Unable to request gpio irq. err=%d\n", error);
		gpio_free(SLCD_AMRI5K_IRQ_GPIO);
		goto err2;
	}

	ts_dev->irq = irq;

	error = request_irq(ts_dev->irq, AMRI5K_s_ts_isr, IRQF_TRIGGER_FALLING, S_TS_DRIVER_NAME, ts_dev);
	if (error) {
...
#ifdef AMRI5K_SHUTDOWN_TOUCH_AFTER_POWER_ON
    //schedule_delayed_work(&ts_dev->touchoffwork, msecs_to_jiffies(1000));
    queue_delayed_work(ts_work_queue, &ts_dev->touchoffwork, msecs_to_jiffies(1000));
    dprintk("-----------------------------------------------------------\n");
    dprintk("Register workqueue AMRI5K_ts_shutdown_touch_chip_work %ld\n",msecs_to_jiffies(1000));
#else
	if(is_folder_open())
	{
        mdelay(700);	
...
//    schedule_delayed_work(&ts_dev->esdwork, msecs_to_jiffies(AMRI5K_ESD_POLLING_TIME + 5000)); // Check start after minimum 5 secs
    queue_delayed_work(ts_work_queue, &ts_dev->esdwork, msecs_to_jiffies(AMRI5K_ESD_POLLING_TIME + 5000)); // Check start after minimum 5 secs
    dprintk("-----------------------------------------------------------\n");
    dprintk("Register workqueue AMRI5K_s_ts_esd_recovery_work \n");
#endif

	is_ts_initialized = 1;

#ifdef CONFIG_HAS_EARLYSUSPEND
		s_ts_early_suspend.suspend = amri5k_s_ts_early_suspend;
		s_ts_early_suspend.resume = amri5k_s_ts_late_resume;
		s_ts_early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN - 40;
		register_early_suspend(&s_ts_early_suspend);
#endif
	
	return error;

 err2:
	input_unregister_device(AMRI5K_s_ts_input);
	AMRI5K_s_ts_input = NULL; /* so we dont try to free it below */
 err1:
	input_free_device(AMRI5K_s_ts_input);
	AMRI5K_s_ts_input = NULL;
	
	return error;
}


static int AMRI5K_s_ts_remove(struct spi_device *spi)
{
	struct AMRI5K_s_ts_device *priv = dev_get_drvdata(&spi->dev);
...
static struct spi_driver AMRI5K_s_ts_driver = {
	.driver = {
		.name  = S_TS_DRIVER_NAME,
		.owner = THIS_MODULE,
	},
	.probe         = AMRI5K_s_ts_probe,
	.remove        = __devexit_p(AMRI5K_s_ts_remove),
};

static int __init AMRI5K_s_ts_init(void)
{
	int ret;	

	printk("AMRI5K Sub Driver Module Init\n");

	ts_work_queue = create_singlethread_workqueue("ts_work_queue");
	if (!ts_work_queue)
		return -ENOMEM;
	
	memset(&AMRI5K_s_ts_dev, 0, sizeof(struct AMRI5K_s_ts_device));
	ret = spi_register_driver(&AMRI5K_s_ts_driver);
	printk("AMRI5K Sub Touch Driver Module Init End: %d\n", ret);
	return ret;
}

static void __exit AMRI5K_s_ts_exit(void)
{
	printk(KERN_ERR"AMRI5K Driver Module Exit\n");
	spi_unregister_driver(&AMRI5K_s_ts_driver);
	if (ts_work_queue)
		destroy_workqueue(ts_work_queue);
}
...
module_init(AMRI5K_s_ts_init);
module_exit(AMRI5K_s_ts_exit);

...
#define USE_IRQ			1
#define GPIO_CARKIT_IRQ 92
#ifdef CONFIG_LGE_MULTIKIT
...
#endif
extern void audio_rx_analog_dock_update(int en);

void audio_set_state(int state)
{
	dock_state = state;
	printk(KERN_INFO"%s: audio_set_state:%d\n",__func__,dock_state);
	audio_rx_analog_dock_update(dock_state);
}

int audio_get_state(void)
{
	printk(KERN_INFO"%s: dock_state:%d\n",__func__,dock_state);
	return dock_state;
}

EXPORT_SYMBOL(audio_get_state);
...
static void hall_ic_dock_work_func(struct work_struct *work)
{
	...
	if(gpio_carkit ==  GPIO_CARKIT_DETECT )
	{
		state = DOCK_STATE_CAR;
	}
	else if(gpio_carkit == GPIO_UNDOCKED)
	{
		state = DOCK_STATE_UNDOCKED;
	}
	else

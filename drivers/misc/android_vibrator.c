...
#if defined(CONFIG_LGE_VIB_USE_WORK_QUEUE)	/* C710 Rev.D */
...
#endif	/* CONFIG_LGE_VIB_USE_WORK_QUEUE */
...
static int android_vibrator_force_set(int nForce)
{
 ...
 int android_vibrator_tune(int nForce)
{
	/* Check the Force value with Max and Min force value */
 ...
 EXPORT_SYMBOL(android_vibrator_tune);

#if defined(CONFIG_LGE_VIB_USE_WORK_QUEUE)
...
#else	/* CONFIG_LGE_VIB_USE_WORK_QUEUE */
...
struct timed_vibrator_data android_vibrator_data = {
	...
	.max_timeout = 36000000, /* max time for vibrator enable 30 sec. */
};
